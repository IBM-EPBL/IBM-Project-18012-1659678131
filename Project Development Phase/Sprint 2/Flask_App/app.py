...
@app.route("/upload_1") # route for uploads
def test_1():
    return render_template("index6.html")#rendering html page


@app.route('/predict-spiral', methods=['GET', 'POST'])
def upload_1():
...
@app.route("/upload_2") # route for uploads
def test_2():
    return render_template("index6_w.html")#rendering html page

@app.route('/predict-wave', methods=['GET', 'POST'])
def upload_2():
    if request.method == 'POST':
        f=request.files['file'] #requesting the file
        basepath=os.path.dirname(__file__)#storing the file directory
        ...
        #Loading the saved model
        print("[INFO] loading model...")
        model = pickle.loads(open('parkinson_w.pkl', "rb").read())
        # pre-process the image in the same manner we did earlier
        image = cv2.imread(filepath)
        output = image.copy()
        # load the input image, convert it to grayscale, and resize
